...
app.get("/api/search", requireRole("player", "dm"), (req, res) => {
  ...
  const limit = Math.min(Math.max(Number.parseInt(String(req.query.limit || "40"), 10) || 40, 1), 100);
  ...
  if (sessionUser.role === "dm") {
    const npcRows = db
      ...
      .all(searchPattern, searchPattern, searchPattern, searchPattern, searchPattern, searchPattern, limit);
    ...
    for (const row of npcRows) {
      results.push({
        ...
        id: row.id,
        title: row.name,
        snippet: [row.rank_title, row.house, row.faction, row.court, row.ring].filter(Boolean).join(" • "),
    ...
    for (const row of personalAliasRows) {
      results.push({
        ...
        id: row.id,
        title: row.alias,
        snippet: `${row.owner_display_name || row.owner_username} • NPC: ${row.npc_name}`,
        url: `/directory/${row.slug}`,
        metadata: {
    ...
    for (const row of npcNoteRows) {
      results.push({
        ...
        snippet: `${row.author_display_name || row.author_username}: ${row.content}`,
        url: `/directory/${row.slug}`,
        metadata: {
          npc_name: row.npc_name,
          npc_slug: row.slug,
    ...
    const suspectRows = db
      .prepare(
        `
          SELECT
            dashboard_suspects.id,
            dashboard_suspects.name,
            dashboard_suspects.status,
            dashboard_suspects.note,
            users.display_name AS owner_display_name,
            users.username AS owner_username
          FROM dashboard_suspects
          INNER JOIN users
            ON users.id = dashboard_suspects.user_id
    ...
    for (const row of suspectRows) {
      results.push({
        ...
        id: row.id,
        title: row.name,
        snippet: `${row.owner_display_name || row.owner_username} • ${row.status}${row.note ? ` • ${row.note}` : ""}`,
        url: "/",
        metadata: {
          status: row.status,
          owner_display_name: row.owner_display_name || null,
    ...
    const dashboardNoteRows = db
      .prepare(
        `
          SELECT
            dashboard_notes.id,
            dashboard_notes.content,
            users.display_name AS owner_display_name,
            users.username AS owner_username
          FROM dashboard_notes
          INNER JOIN users
            ON users.id = dashboard_notes.user_id
    ...
    for (const row of dashboardNoteRows) {
      results.push({
        ...
        id: row.id,
        title: `Personal note • ${row.owner_display_name || row.owner_username}`,
        snippet: row.content,
        url: "/",
        metadata: {
          owner_display_name: row.owner_display_name || null,
    ...
    for (const row of mapPinRows) {
      results.push({
        ...
        id: row.id,
        title: row.title,
        snippet: `${row.owner_display_name || row.owner_username} • ${row.map_layer} • ${row.category}${row.note ? ` • ${row.note}` : ""}`,
        url: "/maps",
        metadata: {
          map_layer: row.map_layer,
          category: row.category,
    ...
    for (const row of recapRows) {
      results.push({
        ...
        id: row.id,
        title: row.title,
        snippet: `Session ${row.session_number} • ${row.content}`,
        url: "/",
        metadata: {
    ...
    for (const row of importLogRows) {
      results.push({
        ...
        id: row.id,
        title: row.filename,
        snippet: `${row.result} • ${row.message}`,
        url: "/dm-tools",
        metadata: {
          result: row.result,
          created_at: row.created_at,
    ...
    const archiveRows = db
      .prepare(
        `
          ...
          WHERE
            LOWER(COALESCE(archive_records.object_type, '')) LIKE ?
    ...
    for (const row of archiveRows) {
      results.push({
        ...
        snippet: `${row.object_type} • ${row.owner_display_name || row.owner_username || "no owner"}${row.archive_reason ? ` • ${row.archive_reason}` : ""}`,
        url: "/archive",
        metadata: {
    ...
    const personalAliasRows = db
      ...
      .all(sessionUser.id, searchPattern, limit);
    ...
    const suspectRows = db
      ...
      .all(sessionUser.id, searchPattern, searchPattern, searchPattern, limit);
    ...
    for (const row of suspectRows) {
      results.push({
        ...
        id: row.id,
        title: row.name,
        snippet: `${row.status}${row.note ? ` • ${row.note}` : ""}`,
        url: "/",
        metadata: {
          status: row.status,
        },
        ...
        `
          ...
          WHERE user_id = ?
            ...
            AND (
              ...
              OR LOWER(COALESCE(map_layer, '')) LIKE ?
            )
          ORDER BY updated_at DESC, id DESC
          LIMIT ?
        `
      )
      .all(sessionUser.id, searchPattern, searchPattern, searchPattern, searchPattern, limit);
    ...
    for (const row of mapPinRows) {
      results.push({
        ...
        id: row.id,
        title: row.title,
        snippet: `${row.map_layer} • ${row.category}${row.note ? ` • ${row.note}` : ""}`,
        url: "/maps",
      });
    }
  ...
  const rankedResults = results
    .map((result) => {
      ...
      return {
        ...result,
        score: startsWith + containsTitle + containsSnippet,
      };
    })
    .sort((a, b) => b.score - a.score || String(a.label).localeCompare(String(b.label)) || String(a.title).localeCompare(String(b.title)))
    .slice(0, limit)
    .map(({ score, ...rest }) => rest);
